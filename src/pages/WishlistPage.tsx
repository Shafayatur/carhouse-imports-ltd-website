...
export default function WishlistPage() {
  ...
  const favoritedCars = ALL_CARS.filter(car => wishlist.includes(car.id));
    ...
    <main className="bg-luxury-black min-h-screen pt-32 selection:bg-gold selection:text-black text-white">
      ...
      <div className="container mx-auto px-6 md:px-12 py-16">
        <div className="space-y-6 mb-24">
          ...
          <h1 className="text-6xl md:text-8xl font-serif italic tracking-tighter">The <span className="not-italic font-display font-medium uppercase text-white/90">Wishlist</span></h1>
          <p className="text-white/40 max-w-xl font-light text-lg">Monitoring real-time market fluctuations and private auction availability for your target acquisitions.</p>
        </div>
              ...
              >
                ...
                <div className="p-8 space-y-6">
                  <div className="flex justify-between items-start">
                    <h3 className="text-2xl font-serif italic text-white group-hover:text-gold transition-colors">{car.name}</h3>
                    <button 
                  ...
                  <div className="flex items-center gap-6 text-[10px] uppercase tracking-widest text-white/30 font-black">
                    <span className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-gold" /> Market Watch Active</span>
                  </div>

                  <div className="pt-6 border-t border-white/5 flex items-center justify-between">
                    <Link to={`/inventory/${car.id}`} className="text-[10px] uppercase tracking-[0.3em] font-black text-white hover:text-gold transition-colors flex items-center gap-3">
