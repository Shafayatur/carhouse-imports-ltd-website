...
export function AIConcierge() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "model", text: "Welcome to the Digital Vault. I am your Intelligence Concierge. Describe your requirements, and I shall unveil the perfect asset from our collection." }
  ]);
  ...
  const vaultContext = ALL_CARS.map(car => (
    `- ${car.brand} ${car.name} (ID: ${car.id}): ${car.category}, ${car.specs}. ${car.description}`
  )).join("\n");

  const systemInstruction = `You are the Digital Concierge for 'Car House Imports', an ultra-luxury automotive sourcing company. 
Your tone is sophisticated, formal, and authoritative. 

VAULT ASSETS:
${vaultContext}

INSTRUCTIONS:
1. Help users find cars based on family needs, speed, track performance, or exclusivity.
2. If we have a match, recommend 1-2 specific cars.
3. IMPORTANT: To display a car card, you MUST include the tag [CAR_CARD: carId] in your response for each recommended car.
...
5. Keep responses concise and elegant.`;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: "model", text: "I apologize, I am unable to access the intelligence network at this moment. Please try again shortly." }]);
    } finally {
      setIsTyping(false);
    }
  };
          ...
          >
            <div className="bg-white/[0.03] border border-white/5 rounded-lg overflow-hidden transition-all duration-500 group-hover:border-gold/30">
               <div className="aspect-[16/10] relative overflow-hidden">
                  <img src={car.image} alt={car.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700" />
                  <div className="absolute top-2 right-2 px-2 py-1 bg-gold text-black text-[8px] font-black uppercase rounded-sm">
                    {car.category}
                  </div>
               </div>
               <div className="p-4">
                  <p className="text-[8px] uppercase tracking-widest text-gold font-bold mb-1">{car.brand}</p>
                  <p className="text-sm font-serif italic text-white group-hover:text-gold transition-colors">{car.name}</p>
                  <div className="flex justify-between items-center mt-2">
                     <p className="text-[10px] text-white/30 font-black uppercase">{car.price}</p>
        ...
        {isOpen && (
          <>
            ...
            >
              <div className="p-6 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-serif italic text-white/50">Digital Intelligence</p>
                  </div>
                </div>
                <button 
                  onClick={() => setIsOpen(false)}
              ...
              <div 
                ref={scrollRef}
                className="flex-1 min-h-0 overflow-y-auto p-6 space-y-8 custom-scrollbar scroll-smooth"
              >
                {messages.map((m, idx) => (
                  <motion.div
                    key={idx}
                  ...
                  >
                    <div className={`max-w-[85%] p-5 rounded-2xl text-[13px] md:text-sm font-medium leading-[1.6] ${
                      m.role === 'user' 
