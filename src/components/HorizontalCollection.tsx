...
export function HorizontalCollection() {
  ...
  const displayCars = ALL_CARS.filter(car => car.id.startsWith("g") || car.id.startsWith("p")).slice(0, 6);
    ...
    <section ref={targetRef} className="relative h-[300vh] bg-luxury-black">
      <div className="sticky top-0 h-screen flex items-center overflow-hidden">
        <motion.div style={{ x }} className="flex gap-12 px-12 md:px-24">
              ...
              <Link to={`/inventory/${car.id}`} className="block h-full">
                <div className="relative w-full h-full overflow-hidden rounded-sm transition-all duration-1000">
                  ...
                  <div className="absolute top-12 left-12 text-white">
                    <p className="text-[10px] uppercase tracking-[0.5em] font-black text-gold mb-3">{car.brand}</p>
                    <h3 className="text-4xl md:text-5xl font-serif italic">{car.name}</h3>
                  </div>
          ...
          {/* Final Call to Action in the scroll */}
          <div className="w-[40vw] flex-shrink-0 flex items-center justify-center border-l border-white/10 ml-12">
             <div className="text-center space-y-8">
               <h3 className="text-4xl md:text-6xl font-display font-light uppercase tracking-tighter">Seek <br /><span className="italic font-serif text-gold">Something Else?</span></h3>
